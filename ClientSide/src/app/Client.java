    ...
    public int requestFile(String fileName) throws IOException {
        ...
        while (true) {
           try {
               ...
               ClientResponder cr = new ClientResponder(answer, socket);
               cr.run();
           } catch (SocketTimeoutException e){
               System.out.println("--Resending last ack");
               DatagramPacket ack = PacketFactory.createAckPacket(iPAddress, port, lastReceived);
               socket.send(ack);
           }
...
private void goBackN(ArrayList<DatagramPacket> packetList) throws IOException {
    ...
    public int requestFile(String fileName) throws IOException {
        ...
        while(true){
            ...
            int packetNo = ClientResponder.getPacketNo(answer);

            socket.send(PacketFactory.createAckPacket(iPAddress, port, packetNo));
        }
